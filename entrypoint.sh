...
rm -rf /audio/output
mkdir -p /audio /tmp /run/mpd /var/lib/mpd/ /music/cache /data
mkfifo /audio/output
...
go-librespot --config_dir /etc/golibrespot/ > /var/log/audio-services/go-librespot.log 2>&1 &
LIBRESPOT_PID=$!
...
while true; do
    ...
    if ! kill -0 "$LIBRESPOT_PID" 2>/dev/null; then
        echo "ERROR: go-librespot process died unexpectedly. Check /var/log/audio-services/go-librespot.log"
        cleanup
        exit 1
    fi
