...
IMAGE_NAME = guidcruncher/vioxdotnet-server
SOLUTION = viox.sln

.PHONY: help build preview up down restart logs status shell clean dotnet-build publish release format debug dev update-packages
...
debug: ## Start the container in debug mode
	$(COMPOSE) up --build --force-recreate
	$(COMPOSE) rm -f
