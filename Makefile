...
.PHONY: help build client clean-container preview up down restart logs status shell clean dotnet-build client-build publish release format debug dev update-packages restore
...
restore: ## Restores packages and npm modules
	cd ./src/AudioHub && npm i
	dotnet restore ./viox.sln
