...
const playlist = ref<MediaMetaData | null>(null)
...
async function loadData(id: string) {
  ...
  try {
    const [playlistRes, tracksRes] = await Promise.all([
      api.library.getLibraryItem('spotify', `spotify:playlist:${id}`),
      api.library.getLibraryItemChildItems('spotify', `spotify:playlist:${id}`),
    ])
    ...
    <div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div v-if="isLoading" class="flex items-center justify-center py-20">
        <div class="flex items-center space-x-3 text-slate-400">
          ...
          <span class="text-sm font-medium">Loading Playlist...</span>
        </div>
      </div>
      ...
      <template v-else>
        ...
        >
          <img
            :src="playlist.imageUrl || '/hifi.png'"
          ...
          <div class="space-y-3 flex-1 min-w-0">
            ...
            <p class="text-slate-400 text-sm sm:text-base font-medium truncate">
              {{ playlist.artist || 'Unknown Author' }}
            </p>
