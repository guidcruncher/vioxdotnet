...
export function useApiClient(baseUrl: string = apiBaseUrl) {
  ...
  const library = {
    ...
    getLibraryItem: (source: string, uri: string): Promise<MediaMetaData> =>
      transport.request<MediaMetaData>(`/api/v1/library/${source}/${uri}`),
    getLibraryItemChildItems: (source: string, uri: string): Promise<MediaMetaData[]> =>
      transport.request<MediaMetaData[]>(`/api/v1/library/${source}/${uri}/items`),
  }
