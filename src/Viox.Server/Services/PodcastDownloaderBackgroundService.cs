    ...
    private async Task ProcessBatchSafelyAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            await Parallel.ForEachAsync(itemsToProcess, parallelOptions, async (podcast, ct) =>
            {
                ...
                DateTimeOffset maxAge = DateTimeOffset.UtcNow.AddHours(-_optionsMonitor.CurrentValue.MaxAgeHours);
                ...
                foreach (MediaMetaData item in items)
                {
                    ...
                    {
                        _logger.LogInformation("Successfully cached title '{Title}' at path: {Path}", item.Title, resultPath);
                    }
                    else
                    {
