...
public class PodcastEpisodeParser
{
    ...
    public async Task<IReadOnlyList<PodcastEpisode>> ParseEpisodesAsync(string id, Stream xmlStream, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var item in channel.Elements("item"))
        {
            ...
            if (DateTimeOffset.TryParse(item.Element("pubDate")?.Value, out var parsedDate))
            {
            ...
            if (!RssDurationParser.TryParseToSeconds(item.Element(ItunesNs + "duration")?.Value ?? "", out durationSeconds))
            {
                durationSeconds = 0;
            }
