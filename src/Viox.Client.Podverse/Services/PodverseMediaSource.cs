...
public class PodverseMediaSource : IMediaSource
{
    ...
    {
        IReadOnlyCollection<MediaMetaData>? src = _favorites.GetAllFavorites();
        if (src is null)
        {
            return [];
        }

        List<MediaMetaData> res = src.Where(t => t.Uri?.Source == "podverse").ToList();
        return await Task.FromResult(res);
    }

    public async Task<IList<MediaMetaData>> ResolveChildItems(MediaUri? parent, string childType, CancellationToken cancellationToken)
    {
        List<MediaMetaData> items = new();

        if (parent is null)
        {
            _logger.LogWarning("Unable to resolve child items: Parent MediaUri is null.");
            return items;
        }
        ...
        Podcast? podcast = await _client.GetPodcastByIdAsync(parent.Id, cancellationToken);
        if (podcast is null)
        {
            return items;
        }
