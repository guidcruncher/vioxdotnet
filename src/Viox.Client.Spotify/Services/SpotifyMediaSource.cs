...
public class SpotifyMediaSource : IMediaSource
{
    ...
    public async Task<MediaMetaData?> ResolveMetaData(MediaUri? uri, CancellationToken ct = default)
    {
        ...
        {
            case "album":
                SpotifyResponse<SpotifyAlbum>? rAlbum = await _client.GetAlbumAsync(uri.ToString(), null, ct);
            ...
            case "track":
                SpotifyResponse<SpotifyTrack>? rTrack = await _client.GetTrackAsync(uri.ToString(), null, ct);
            ...
            case "show":
                SpotifyResponse<SpotifyShow>? rShow = await _client.GetShowAsync(uri.ToString(), null, ct);
            ...
            case "episode":
                SpotifyResponse<SpotifyEpisode>? rEpisode = await _client.GetEpisodeAsync(uri.ToString(), null, ct);
            ...
            case "playlist":
                SpotifyResponse<SpotifyPlaylist>? rPlaylist = await _client.GetPlaylistAsync(uri.ToString(), null, null, null, ct);
