...
public sealed class FileAuthTokenStore : IAuthTokenStore
{
    ...
    public async Task SaveUserNameAsync(string username, CancellationToken cancellationToken = default)
    {
        TokenData? t = await GetTokenAsync(cancellationToken);
        if (t is not null)
        {
            t.UserName = username;
            await SaveTokenAsync(t, cancellationToken);
        }
    }
