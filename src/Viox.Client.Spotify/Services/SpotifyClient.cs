...
public sealed class SpotifyClient : ISpotifyClient
{
        ...
        ILogger<SpotifyClient> logger)
    {
        ...
        _json = new JsonSerializerOptions
        {
            ...
            Converters = { new PlaylistItemConverter() }
        };
    ...
    private async Task<SpotifyResponse<T>> SendRawAsync<T>(HttpMethod method, string requestUri, HttpContent? content, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (response.IsSuccessStatusCode)
            {
                ...
                var data = await response.Content.ReadFromJsonAsync<T>(_json, cancellationToken: cancellationToken).ConfigureAwait(false);
            ...
            try
            {
                var errorWrapper = await response.Content.ReadFromJsonAsync<SpotifyErrorWrapper>(_json, cancellationToken: cancellationToken).ConfigureAwait(false);
                errorObject = errorWrapper?.Error;
                _logger.LogError("Spotify API returned an error for URL: {Url}. Status Code: {StatusCode}, Error: {Error}", requestUri, (int)response.StatusCode, errorObject);
            }
            catch
            {
