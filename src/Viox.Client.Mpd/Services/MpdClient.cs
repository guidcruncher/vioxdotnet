...
public sealed class MpdClient : IMpdClient, IAsyncDisposable, IDisposable
{
    ...
    /// <summary>
    /// Updates the entire MPD music database and waits until the update is complete.
    /// </summary>
    /// <param name="cancellationToken">A cancellation token to cancel the operation.</param>
    /// <returns>A task representing the asynchronous update operation.</returns>
    public Task UpdateAsync(CancellationToken cancellationToken = default) => UpdateDirectoryAsync(string.Empty, cancellationToken);

    /// <summary>
    /// Updates a specific directory in the MPD music database relative to <c>music_directory</c> and waits until the update is complete.
    /// </summary>
    /// <param name="path">The relative path to update, or empty string to scan the entire root database.</param>
    /// <param name="cancellationToken">A cancellation token to cancel the operation.</param>
    /// <returns>A task representing the asynchronous update operation.</returns>
    public async Task UpdateDirectoryAsync(string path, CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Initiating MPD update with wait for path: '{Path}'", string.IsNullOrWhiteSpace(path) ? "ROOT" : path);
        ...
        else
        {
            ...
            sb.AppendLine($"update \"{escapedPath}\"");
        }

        sb.AppendLine("idle database");
        sb.AppendLine("command_list_end");

        await SendCommandAsync(sb.ToString().TrimEnd(), cancellationToken).ConfigureAwait(false);
        _logger.LogInformation("MPD update completed for path: '{Path}'", string.IsNullOrWhiteSpace(path) ? "ROOT" : path);
    }
